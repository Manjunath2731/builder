...
import { getYourRegistrations } from 'src/slices/channelPartner';
// import "./index.css";

const ChannelRegistrations = ({brokerId}) => {
  const dispatch = useDispatch();

  const registrationData = useSelector((state) => state.channelPartner?.yourregistration);

  useEffect(async () => {
    dispatch(getYourRegistrations(brokerId));
  }, []);
  ...
  const series = [
    {
      ...
      data: registrationData?.past12MonthsTotalss?.map(i => i.count),
      color: '#CF99CA'
    },
    // {
...
//   });
  ...
  return (
    <Box sx={{ py: 2 }}>
      ...
      <Typography sx={{ fontWeight: 'bold',fontSize:"13px" }}>REGISTRATIONS</Typography>
      <Card sx={{ mt: 1.5 ,height:"auto" }} id="channel--resg" >
        <Box>
          ...
          <Typography variant="h6" style={{ fontSize: "12px", marginLeft: "10px", marginTop: "20px" }}>Total</Typography>
                <Typography  style={{ fontSize: "20px", marginLeft: "10px",fontWeight:"bold"}}>{registrationData?.totalregistartion}</Typography>
...
export default ChannelRegistrations;